...
parser <- OptionParser()
parser <- add_option(parser, c("--res"), type = "character")  # fine-mapping result
parser <- add_option(parser, c("--sumstats"), type = "character")  # fine-mapping sum stats
parser <- add_option(parser, c("--gene-info"), type = "character")  # gene location (by TSS)
