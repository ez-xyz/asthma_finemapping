...
parser <- OptionParser()
parser <- add_option(parser, c("--gwas"), type = "character", default = "")  # GWAS summary statistics
parser <- add_option(parser, c("--prior"), type = "character", default = "")  # prior
parser <- add_option(parser, c("--locus"), type = "character", default = "")  # fine-map locus
parser <- add_option(parser, c("--ldr"), type = "character", default = "")  # LD matrix
parser <- add_option(parser, c("--rvar"), type = "character", default = "")  # LD snp information
...
for (i in finemap_locus) {
  ...
  colnames(vars) <- c("chr", "snp", "pos", "a1", "a0")
