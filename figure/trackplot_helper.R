...
finemapping_annot_trackplot <- function(finemapstats,
  ...
  pip.track <- DataTrack(range = pip.gr,
                         ...
                         col = c("#A29C68", "#6A89A7"),  # specify colors for different loci (#0072b2 is the default blue color for single locus)
                         name = "PIP",
                         legend = FALSE)
  ...
  if(!missing(HiC_loops) && (length(HiC_loops) > 0)){
    dpars.HiC <- list(col.interactions = "lightblue",
                      col.anchors.fill = "royalblue",
  ...
  dpars.genes <- list(col.title = "black",
                      ...
                      frame = FALSE,
                      cex.axis = 1,
                      fontsize = 12,  # size of track name
                      background.title = "transparent")
  ...
  track.sizes <- c(0.3,
                   0.3,
                   rep(0.05, length(countsdata.tracks)),
                   rep(0.05, length(peaks.tracks)),
                   rep(0.2, length(HiC_loops.tracks)),
                   0.2,
                   0.2)
  ...
  if( !missing(highlight_snps) && (length(highlight_snps) > 0)){
    ...
    if(length(highlight_pos) == 1){
      list.of.tracks <- HighlightTrack(trackList = list.of.tracks,
                                       start = c(highlight_pos - 1000), width = 2000,
...
knownGeneObject <- function(region, genome, txdb, keytype = "ENTREZID", track_name = "Genes"){
  ...
  grtrack <- grtrack[-which(grtrack@range$feature == "ncRNA")]
...
# get gene region
get_gene_region <- function(gene.mapping.res, genes.of.interest, pip.thresh = 0.2, ext = 10000, select.region = c("all", "locus")){
  
  select.region <- match.arg(select.region)
  
  high.conf.snp.df <- gene.mapping.res |> dplyr::filter(pip > pip.thresh) |>
